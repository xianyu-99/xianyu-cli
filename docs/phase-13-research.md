...
> 调研日期：2026-05-22
> 调研目标：为 YuCLI 第 13 期"Chrome DevTools MCP"选择最优技术方案
...
用户已启动 Chrome（如 `chrome --remote-debugging-port=9222`），YuCLI 通过 HTTP 发现端点获取 WebSocket URL 后连接。
...
```
MCP Client (YuCLI) --stdio/HTTP--> MCP Server --Puppeteer/Playwright--> CDP --WebSocket--> Chrome
```
...
**缺点**：
- 引入额外依赖（Jackson 2.18 可能与 YuCLI 的 Jackson 2.16 冲突，需验证）
...
**本质**：复用 YuCLI 已有的 Jackson + OkHttp，手写 WebSocket 客户端直接与 Chrome CDP 通信。

**需要实现的组件**：

```
com.yucli.mcp.browser/
...
- 与 YuCLI 现有 MCP 框架风格一致（手写 JSON-RPC）
- 包体积零增加
- 教育价值高（契合 YuCLI"从零手写"的定位）
...
1. **契合 YuCLI 项目定位**：YuCLI 是一个"从零手写"的教育型 Agent CLI 产品，第 10-11 期已经手写了完整的 MCP 协议栈（JSON-RPC、stdio transport、Streamable HTTP）。直接手写 CDP 与这一理念完全一致，能让用户理解浏览器自动化的底层原理。

2. **零额外依赖**：YuCLI 已有 Jackson 2.16 + OkHttp 4.12。Java 11+ 内置 `java.net.http.WebSocket`（YuCLI 用 Java 17），不需要引入任何新库。
...
```json
{
  "mcpServers": {
    "browser": {
      "command": "java",
      "args": ["-jar", "YuCLI-browser-mcp.jar"]
    }
  }
}
```

Browser MCP Server 是一个独立的 Java 进程，通过 stdio 与 YuCLI 通信。这种方式：
- 复用第 10 期全部基础设施（`McpServerManager`、`StdioTransport`、`JsonRpcClient`）
- 浏览器进程隔离在 MCP Server 子进程中，崩溃不影响 YuCLI 主进程
- 符合 MCP 生态标准，可被其他 MCP Client 复用

**模式 B：作为 YuCLI 内置模块（备选）**

直接在 YuCLI 内部启动 Chrome 进程，工具注册到 `ToolRegistry`。这种方式：
- 无需额外进程，启动更快
- 但浏览器崩溃会影响 YuCLI 主进程
- 与 MCP 生态解耦

**推荐模式 A**，与 YuCLI 现有 MCP 架构保持一致。
...
1. **WebSocket 客户端**：复用 YuCLI 第 10 期的 `JsonRpcClient` 设计模式，但 transport 改为 WebSocket 而非 stdio/HTTP。`CdpWebSocketTransport` 实现 `McpTransport` 接口（或独立设计）。
...
- [YuCLI 第 10 期 MCP 协议核心](phase-10-mcp-core.md)
- [YuCLI 第 11 期 MCP 高级能力](phase-11-mcp-advanced.md)
